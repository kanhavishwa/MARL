...
import { encodeState } from "./qlearning";
export const TICK_MS = 100;
export const ticksToSeconds = (t) => (t * TICK_MS) / 1000;
...
const EXIT_LINE = 0.6; // car has cleared the intersection box
export class TrafficSim {
  ...
  spawn() {
    Object.keys(this.cfg.spawnRate).forEach((d) => {
      if (Math.random() < this.cfg.spawnRate[d]) {
        const inLane = this.cars.filter((c) => c.dir === d).length;
        if (inLane < 16) {
          const last = this.cars.filter((c) => c.dir === d).reduce((m, c) => Math.min(m, c.pos), 1);
          if (last > MIN_GAP) {
            this.cars.push({
              id: this.nextId++,
              dir: d,
      ...
      for (const car of lane) {
        ...
        if (car.turn !== "straight") {
          if (car.pos > ENTER_LINE && car.pos < EXIT_LINE) {
            car.turnProgress = Math.min(1, (car.pos - ENTER_LINE) / (EXIT_LINE - ENTER_LINE));
  ...
  phaseLabel() {
    ...
    switch (this.phase) {
      case 0:
        return "N–S STRAIGHT";
      case 1:
        return "N–S LEFT ARROW";
      case 2:
        return "E–W STRAIGHT";
      case 3:
        return "E–W LEFT ARROW";
    }
  }
  tick() {
    this.step += 1;
