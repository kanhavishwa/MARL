...
import { TrafficSim, TICK_MS, ticksToSeconds } from "@/lib/traffic/simulation";
import { QLearningAgent } from "@/lib/traffic/qlearning";
import { Button } from "@/components/ui/button";
import { Slider } from "@/components/ui/slider";
const PRESETS = {
  Balanced: {
    spawn: { N: 0.2, S: 0.2, E: 0.2, W: 0.2 },
  ...
  "Rush hour (E-W)": {
    spawn: { N: 0.15, S: 0.12, E: 0.45, W: 0.4 },
    turnMix: { straight: 0.78, left: 0.1, right: 0.12 },
    description: "Morning commute on the main E-W corridor.",
  },
  "Indian chowk": {
    spawn: { N: 0.55, S: 0.5, E: 0.55, W: 0.52 },
    turnMix: { straight: 0.45, left: 0.3, right: 0.25 },
    description: "Dense mixed traffic, lots of turns — classic chowk gridlock.",
  },
  "Market street": {
    spawn: { N: 0.3, S: 0.32, E: 0.42, W: 0.38 },
    turnMix: { straight: 0.5, left: 0.28, right: 0.22 },
    description: "Heavy turning into shops on a busy bazaar road.",
  },
  "Highway merge": {
    spawn: { N: 0.55, S: 0.55, E: 0.12, W: 0.1 },
    turnMix: { straight: 0.85, left: 0.07, right: 0.08 },
    description: "Fast N-S throughput, minor cross street.",
  },
  "T-junction (W blocked)": {
    spawn: { N: 0.3, S: 0.3, E: 0.4, W: 0.0 },
    turnMix: { straight: 0.55, left: 0.25, right: 0.2 },
    description: "Three-way flow — west approach closed.",
  },
  Gridlock: {
    spawn: { N: 0.5, S: 0.5, E: 0.5, W: 0.5 },
    turnMix: { straight: 0.6, left: 0.2, right: 0.2 },
    description: "Saturated from every side — stress test.",
  },
};
export function SimulationPanel() {
  const agentRef = useRef(new QLearningAgent());
  const simRef = useRef(new TrafficSim(agentRef.current));
  const [running, setRunning] = useState(true);
  const [speed, setSpeed] = useState(2);
  const [mode, setMode] = useState("rl");
  const [metrics, setMetrics] = useState(simRef.current.getMetrics());
    ...
    () => [
      {
        ...
        hint: "Total simulation ticks since reset (1 tick ≈ 100 ms).",
      },
      {
        label: "Cars passed",
        value: metrics.totalPassed.toLocaleString(),
        hint: "Total vehicles that have crossed the intersection.",
      },
      {
        label: "Avg wait (sec)",
        value: ticksToSeconds(metrics.avgWait).toFixed(2) + "s",
        hint: "Average real time each completed car spent stopped.",
      },
      {
        label: "Live wait (sec)",
        value: ticksToSeconds(metrics.currentWait).toFixed(2) + "s",
        hint: "Mean wait time of cars currently queued.",
      },
      {
        label: "Live queue",
        value: metrics.currentQueue.toString(),
        hint: "How many cars are stopped at the stop line right now.",
      },
      {
        label: "Phase switches",
        value: metrics.switches.toString(),
        hint: "How many times the lights have changed.",
      },
      {
        label: "Cumulative reward",
        value: metrics.reward.toFixed(1),
        hint: "Sum of rewards earned by the agent.",
      },
      {
        label: "ε (exploration)",
        value: metrics.epsilon.toFixed(3),
        hint: "Probability of a random action.",
      },
      {
        label: "Q-states learned",
        value: metrics.qStates.toString(),
        hint: "Distinct (queues, phase) states stored.",
      },
    ],
    [metrics],
  );
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            size="sm"
            variant="outline"
            onClick={() => setMode((m) => (m === "rl" ? "fixed" : "rl"))}
          >
          ...
          <Button size="sm" variant="ghost" onClick={handleNewAgent}>
            New agent
          </Button>
        </div>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-[11px] font-mono">
              ...
              <span className="text-ink">
                S {(turnMix.straight * 100).toFixed(0)}% · L {(turnMix.left * 100).toFixed(0)}% · R{" "}
                {(turnMix.right * 100).toFixed(0)}%
              </span>
            </div>
            <Slider
    ...
    - 0.5 if action = SWITCH`}
        ...
        <div className="border ink-rule rounded-md p-3 bg-paper">
          ...
          <ul className="text-xs leading-relaxed mt-2 space-y-1 text-ink">
            <li>
              <span
                className="inline-block w-3 h-3 align-middle mr-2 rounded-sm"
                style={{ background: "oklch(0.7 0.16 25)" }}
              />{" "}
              Left turn
            </li>
            <li>
            ...
            <li>
              <span
                className="inline-block w-3 h-3 align-middle mr-2 rounded-sm"
                style={{ background: "oklch(0.7 0.12 140)" }}
              />{" "}
              Straight (varied hue)
            </li>
          </ul>
