...
function carScreenPos(car, w, h, cx, cy, roadW) {
  const laneOffset = roadW * 0.25;
  const ENTER = 0.45;
  const EXIT = 0.6;
  const p = car.pos;
  // Approach line position at parameter `pp` along the incoming road
  const approachAt = (dir, pp) => {
    switch (dir) {
      case "N":
        return { x: cx - laneOffset, y: pp * h, rot: Math.PI / 2 };
      case "S":
        return { x: cx + laneOffset, y: h - pp * h, rot: -Math.PI / 2 };
      case "E":
        return { x: w - pp * w, y: cy - laneOffset, rot: Math.PI };
      case "W":
        return { x: pp * w, y: cy + laneOffset, rot: 0 };
    }
  };
  // Outgoing lane position at parameter `pp` (for pp in [EXIT..1.1])
  const exitAt = (dir, turn, pp) => {
    let outDir = dir;
    if (turn === "left") outDir = { N: "W", S: "E", E: "N", W: "S" }[dir];
    else if (turn === "right") outDir = { N: "E", S: "W", E: "S", W: "N" }[dir];
    const tt = Math.min(1, Math.max(0, (pp - EXIT) / (1.1 - EXIT)));
    const dist = roadW / 2 + tt * (Math.min(w, h) / 2);
    switch (outDir) {
      case "N":
        return { x: cx + laneOffset, y: cy - dist, rot: -Math.PI / 2 };
      case "S":
        return { x: cx - laneOffset, y: cy + dist, rot: Math.PI / 2 };
      case "E":
        return { x: cx + dist, y: cy + laneOffset, rot: 0 };
      case "W":
        return { x: cx - dist, y: cy - laneOffset, rot: Math.PI };
    }
  };
...
export function IntersectionCanvas({ sim, size = 480 }) {
